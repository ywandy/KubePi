...
export default {
      ...
      buttons: [
        {
          ...
          disabled: () => {
            return  !checkPermissions({resource: "groups", verb: "update"})
          }
        },
        {
          ...
          disabled: (row) => {
            return row.builtIn || !checkPermissions({resource: "groups", verb: "delete"})
          }
        },
      ],
      paginationConfig: {
  ...
  methods: {
    ...
    search(conditions) {
      ...
      searchGroups(currentPage, pageSize, conditions).then(data => {
        this.loading = false
    ...
    onDelete(name) {
      this.$confirm(this.$t("commons.confirm_message.delete"), this.$t("commons.message_box.alert"), {
        ...
        this.ps = []
        if (name) {
          this.ps.push(deleteGroup(name))
        } else {
          if (this.selects.length > 0) {
            for (const select of this.selects) {
              this.ps.push(deleteGroup(select.name))
            }
          }
        }
        if (this.ps.length !== 0) {

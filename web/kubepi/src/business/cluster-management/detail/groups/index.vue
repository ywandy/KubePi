<template>
  <layout-content>
    ...
    <el-dialog
        ...
        :title="$t('commons.button.'+operation) + '用户组'"
...
import {createClusterGroups, listClusterGroups, listNamespaces} from "@/api/clusters"
import {listGroups} from "@/api/groups"
import {listClusterRoles, deleteClusterGroups, getClusterGroups, updateClusterGroups} from "@/api/clusters";
...
export default {
  ...
  methods: {
    list() {
      this.loading = false
      listClusterGroups(this.name).then(data => {
        this.loading = false
        this.data = data.data
      })
    },
    ...
    onEdit(row) {
      ...
      getClusterGroups(this.name, row.name).then(data => {
    ...
    onDelete(raw) {
      this.$confirm(this.$t("commons.confirm_message.delete"), this.$t("commons.message_box.alert"), {
        ...
        this.ps = []
        if (raw) {
          this.ps.push(deleteClusterGroups(this.name, raw.name, raw.kind))
        } else {
          if (this.selects.length > 0) {
            for (const select of this.selects) {
              this.ps.push(deleteClusterGroups(this.name, select.name, select.kind))
            }
          }
        }
        if (this.ps.length !== 0) { 
    ...
    onConfirm() {
      ...
      switch (this.operation) {
        case "create":
          createClusterGroups(this.name, req).then(() => {
          ...
          break;
        case "edit":
          updateClusterGroups(this.name, this.memberForm.userName, req).then(() => {
